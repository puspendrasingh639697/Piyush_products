...
const NotificationBell = () => {
    ...
    useEffect(() => {
        // ✅ FIX — token nahi hai toh call mat karo
        ...
        const interval = setInterval(() => {
            ...
            if (t) dispatch(fetchNotifications());
        }, 30000);
    ...
    // ✅ Token nahi hai toh bell dikhao hi mat
    const token = localStorage.getItem('token');
                        ...
                        <div className="max-h-64 sm:max-h-80 md:max-h-96 overflow-y-auto">
                            {isLoading && notifications.length === 0 ? (
                                ...
                                notifications.map((notif) => (
                                    <div 
                                        key={notif._id}
                                        className={`p-3 sm:p-4 border-b hover:bg-gray-50 cursor-pointer transition ${!notif.isRead ? 'bg-red-50 border-l-4 border-l-red-600' : ''}`}
                                        onClick={() => handleNotificationClick(notif)}
                                    >
                                        <div className="flex items-start gap-2 sm:gap-3">
                                            <div className={`w-2 h-2 rounded-full mt-1.5 ${!notif.isRead ? 'bg-red-600' : 'bg-gray-300'}`}></div>
